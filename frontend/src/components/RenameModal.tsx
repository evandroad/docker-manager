...
export default function RenameModal({ currentName, onConfirm, onCancel }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={onCancel}>
      <div className="bg-zinc-900 rounded-lg w-96 border border-zinc-700" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-zinc-700 font-bold">Rename Container</div>
        <form className="p-4 flex flex-col gap-4" onSubmit={e => { e.preventDefault(); if (name.trim()) onConfirm(name.trim()) }}>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="px-4 py-2 text-sm bg-blue-600 border-none rounded-md text-white cursor-pointer hover:bg-blue-500" disabled={!name.trim()}>Rename</button>
          </div>
        </form>
      </div>
    </div>
